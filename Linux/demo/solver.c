...
int main(void)
{
	...
	//for(pu32=context;(void*)pu32<context_end;pu32+=1024)*pu32 = 0;

    puts("input.bin not found, use sample data (beta1 testnet block 2)");
    memcpy(inputheader, testinput, 140);

	int clock_time = clock_gettime(CLOCK_MONOTONIC, &time0);
    printf("Clock Time: %i\nRunning Solver...\n", clock_time);
	...
	//solution_count = EhSolver(context, *(uint32_t *)(inputheader+136));
	...
	j = solution_count < 4 ? solution_count : 4;
	for(i=0;i<j;i++)
    {
